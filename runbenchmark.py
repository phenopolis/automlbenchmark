...
from amlb.utils import config_load, datetime_iso, str2bool, str_sanitize, zip_path
...
parser.add_argument(
    ...
    metavar="input_dir",
    default=None,
    help="Folder from where the datasets are loaded by default." f"\n(default: '{default_dirs.input_dir}')",
)
parser.add_argument(
    "-o",
    "--outdir",
    metavar="output_dir",
    default=None,
    help="Folder where all the outputs should be written." f"(default: '{default_dirs.output_dir}')",
)
parser.add_argument(
    "-u",
    "--userdir",
    metavar="user_dir",
    default=None,
    help="Folder where all the customizations are stored." f"(default: '{default_dirs.user_dir}')",
)
parser.add_argument(
    "-p",
    "--parallel",
...
log_dir = amlb.resources.output_dirs(args.outdir or default_dirs.output_dir, session=sid, subdirs="logs", create=True)[
    "logs"
]
...
log_levels = ns(
    ...
    if ":" in args.logging
    else dict(console=args.logging.upper(), app=args.logging.upper(), root=args.logging.upper())
    if args.logging
    else {}
) | ns(
...
try:
    ...
    if not args.keep_scores and args.mode != "local":
        log.warning(
            "`keep_scores` parameter is currently ignored in %s mode, scores are always saved in this mode.", args.mode
        )
