...
small_config_url = (
    "https://raw.githubusercontent.com/openml/automlbenchmark/master/resources/benchmarks/small-8c4h.yaml"
)
medium_config_url = (
    "https://raw.githubusercontent.com/openml/automlbenchmark/master/resources/benchmarks/medium-8c4h.yaml"
)
...
autosklearn_tids = set(
    [int(row[0]) for row in binary_configuration["data"]] + [int(row[0]) for row in multiclass_configuration["data"]]
)
...
benchmark_dids = {try_get_did_for_task(tid) for tid in benchmark_tids if tid is not None}
autosklearn_dids = {try_get_did_for_task(tid) for tid in autosklearn_tids if tid is not None}
