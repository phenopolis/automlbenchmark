...
class Scoreboard:
    ...
    def from_file(cls, path):
        ...
        return cls(
            framework_name=framework_name, benchmark_name=benchmark_name, task_name=task_name, scores_dir=scores_dir
        )
    ...
    def save_df(data_frame, path, append=False):
        ...
        write_csv(data_frame, path=path, header=new_file, index=not is_default_index, append=not new_file)
    ...
    def __init__(self, scores=None, framework_name=None, benchmark_name=None, task_name=None, scores_dir=None):
        ...
        self.scores_dir = (
            scores_dir if scores_dir else output_dirs(rconfig().output_dir, rconfig().sid, ["scores"]).scores
        )
    ...
    def as_data_frame(self):
        ...
        index = []
        df = self.scores if is_data_frame(self.scores) else to_data_frame([dict(sc) for sc in self.scores])
        if df.empty:
        ...
        metrics_cols = [
            ...
            if (col in dir(ClassificationResult) or col in dir(RegressionResult)) and not col.startswith("_")
        ]
        metrics_cols.sort()
        dynamic_cols = [
            col for col in df.columns if col not in index and col not in fixed_cols and col not in metrics_cols
        ]
        dynamic_cols.sort()
    ...
    def as_printable_data_frame(self, verbosity=3):
        ...
        cols = (
            ...
            if verbosity == 1
            else ["id", "task", "fold", "framework", "constraint", "result", "metric", "duration", "seed", "info"]
            if verbosity == 2
            else slice(None)
        )
        return df.loc[:, cols]
...
class Result:
    ...
    def evaluate(self, metric):
        ...
        if hasattr(self, metric):
            ...
            eval_res += Namespace(
                value=nan, higher_is_better=None, message=f"Unsupported metric `{metric}` for {pb_type} problems"
            )
        return eval_res
