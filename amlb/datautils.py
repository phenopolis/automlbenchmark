...
important
    ...
    until replacement by simpler/lightweight versions to avoid potential version conflicts with libraries imported
    by benchmark frameworks.
...
from sklearn.metrics import (  # just aliasing
    accuracy_score,
    auc,
...
class Encoder(TransformerMixin):
    """
    Overly complex "generic" encoder that can handle missing values, auto encoded format
    (e.g. int for target, float for predictors)...
    Should never have written this, but does the job currently.
    However, should think about simpler single-purpose approach.
    """
    ...
    def __init__(
        ...
        :param missing_policy: one of ['ignore', 'mask', 'encode'].
            ignore: can be safely used only if there's no missing value in the data to be transformed,
                    otherwise it may raise an error during transform().
        ...
        :param missing_replaced_by: the value used to replace missing values before encoding.
                                    ...
                                    If using the 'encode' strategy, all missing values will be replaced with
                                        encode(missing_replaced_by).
        :param normalize_fn: if provided, function applied to all elements during fit and transform
                                 (for example, trimming spaces, lowercase...).
        """
        super().__init__()
    ...
    def fit(self, vec):
        ...
        self.classes = (
            np.unique(vec) if self._ignore_missing else np.unique(np.insert(vec, 0, self.missing_replaced_by))
        )
...
def impute_array(X_fit, *X_s, missing_values=np.NaN, strategy="mean"):
    ...
    :param strategy: 'mean' (default) -> missing values are imputed with the mean value of the corresponding vector.
                     ...
                     ('constant', value) -> missing values are imputed with the constant value provided as
                                            the second term of the tuple.
...
def impute_dataframe(X_fit: pd.DataFrame, *X_s: pd.DataFrame, missing_values=np.NaN, strategy="mean"):
    ...
    :param strategy: 'mean' (default) -> missing values are imputed with the mean value of the corresponding vector.
                     'median' -> missing values are imputed with the median value of the corresponding vector.
                     'mode' -> missing values are imputed with the mode of the corresponding vector.
                     ('constant', value) -> missing values are imputed with the constant value provided
                                            as the second term of the tuple.
                     None -> no-op (for internal use).
                     { type: strategy } -> (not ready yet!) each column/feature type will be applied a different
                                           strategy as soon as it is listed in the dictionary.
                                           type must be one of:
                                               (int, float, number, bool, category, string, object, datetime)
