...
from ..utils import as_list, lazy_property, list_all_files, memoize, path_from_split, profile, repr_def, split_path
...
class FileLoader:
    ...
    def _extract_train_test_paths(self, dataset, fold=None):
        if isinstance(dataset, (tuple, list)):
            ...
            return dict(
                train=[
                    self._extract_train_test_paths(p)["train"][0] if i == fold else None
...
class FileDataset(Dataset):
    def __init__(
        ...
        features: List[Union[ns, str]] = None,
        type: str = None,
    ):
        super().__init__()
        self._train = train
...
class ArffDataset(FileDataset):
    def __init__(self, train_path, test_path, target=None, features=None, type=None):
        ...
        super().__init__(
            ArffDatasplit(self, train_path), ArffDatasplit(self, test_path), target=target, features=features, type=type
        )
...
class ArffDatasplit(FileDatasplit):
    ...
    def load_metadata(self):
        ...
        features = [Feature(i, attr[0], to_feature_type(attr[1])) for i, attr in enumerate(attrs)]
    ...
    def load_data(self):
        ...
        dt_conversions = {f.name: f.data_type for f in self.dataset.features if f.data_type == "category"}
        if dt_conversions:
...
class CsvDatasplit(FileDatasplit):
    ...
    def _unique_values(self, col_name: str):
        ...
        return sorted(dt.categories.values if hasattr(dt, "categories") else self._ds[col_name].unique())
...
class ArffConverter(FileConverter):
    ...
    def _write_file(self, df, path):
        ...
        with open(path, "w") as file:
            ...
            attributes = [
                (
                    ...
                    (
                        ...
                        if pat.is_numeric_dtype(dt)
                        else sorted(dt.categories.values)
                        if pat.is_categorical_dtype(dt)
                        else "STRING"
                    ),
                )
            ...
            arff.dump(dict(description=description, relation=name, attributes=attributes, data=df.values), file)
